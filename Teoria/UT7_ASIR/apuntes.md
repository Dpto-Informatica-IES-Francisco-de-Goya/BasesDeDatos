...
## Comprobamos que no hay "coste_eur"
...
## Comprobamos que sí hay "coste_eur"
...
## Comprobamos que sí hay "coste_eur"
mysql -u admin -p1234 logistica_global -e "select coste_eur from mantenimientos_flota limit 2"

## Comprobamos que no hay almacenes duplicados
mysql -u admin -p1234 logistica_global -e "select cod_almacen,count(id) from almacenes group by cod_almacen having count(id) > 1 limit 2;"

## Comprobamos que hay envíos con clientes que no existen
mysql -u admin -p1234 logistica_global -e "select count(*) as num_invalidos from envios where cliente_id not in (select id from clientes);"

## Restauramos la incremental del ejercicio 3
mysql -u admin -p1234 logistica_global < 03_incremental_ejercicio3.sql 

## Comprobamos que NO hay envíos con clientes que no existen
mysql -u admin -p1234 logistica_global -e "select count(*) as num_invalidos from envios where cliente_id not in (select id from clientes);"
```
